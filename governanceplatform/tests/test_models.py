...
def test_user_group(populate_db):
    ...
    for user in users:
        if not user_in_group(user, "OperatorAdmin") and not user_in_group(
            user, "OperatorUser"
        ):
            assert (
                len(user.get_companies()) == 0
            ), f"User {user.email} should not have company(ies)"
        if not user_in_group(user, "ObserverUser") and not user_in_group(
            user, "ObserverAdmin"
        ):
            assert (
                len(user.get_observers()) == 0
            ), f"User {user.email} should not have observer(s)"
        if not user_in_group(user, "RegulatorUser") and not user_in_group(
            user, "RegulatorAdmin"
        ):
            assert (
                len(user.get_regulators()) == 0
...
def test_user_functionnalities(populate_db):
    ...
    for user in users:
        if (
            ...
            assert (
                len(user.get_module_permissions()) == 0
